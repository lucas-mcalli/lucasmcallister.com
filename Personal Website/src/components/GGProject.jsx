...
const GGProject = ({getGraphic, projectTwoElementsRef, showProjectTwo}) => {
  ...
  const contextInView = useInView(contextRef)
  const problemInView = useInView(problemRef)
  const researchInView = useInView(researchRef)
  const designInView = useInView(designRef)
  const handoffInView = useInView(handoffRef)
  ...
  return (
    <>
      <Timeline ref={el => projectTwoElementsRef.current[2] = el} contextInView={contextInView} problemInView={problemInView} researchInView={researchInView} designInView={designInView} handoffInView={handoffInView} />
      <div className="project-two flex gap-9 flex-col relative">
      <section id="CONTEXT" ref={el => {
      ...
      <section id="HANDOFF" ref={el => {
        projectTwoElementsRef.current[5] = el;
        handoffRef.current = el;
      }}>
        <div className="flex flex-col gap-4 md:gap-6">
          <div className='flex flex-col gap-2'>
            <p className="mt-10 lg:mt-18 text-sm">HANDOFF</p>
            <p className="font-serif sm:text-2xl lg:text-3xl xl:text-4xl xl:w-[80%]">Delivering a complete design system and documentation for implementation.</p>
          </div>
          <div className="flex flex-col gap-8">
            <p className="font-sans text-xs xl:w-[80%] md:text-base lg:text-lg">Add your handoff content here...</p>
          </div>
        </div>
      </section>
    </div>
